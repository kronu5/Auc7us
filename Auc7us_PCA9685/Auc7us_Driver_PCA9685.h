...
#else
const byte cCoxaPin[] PROGMEM = {
  cRRCoxaPin,  cRMCoxaPin,  cRFCoxaPin,  cLRCoxaPin,  cLMCoxaPin,  cLFCoxaPin};   //Coxa Motors Signal Pins
const byte cFemurPin[] PROGMEM = {
  cRRFemurPin, cRMFemurPin, cRFFemurPin, cLRFemurPin, cLMFemurPin, cLFFemurPin};  //Femur Motors Signal Pins
const byte cTibiaPin[] PROGMEM = {
  cRRTibiaPin, cRMTibiaPin, cRFTibiaPin, cLRTibiaPin, cLMTibiaPin, cLFTibiaPin};  //Tibia Motors Signal Pins
#ifdef c4DOF
const byte cTarsPin[] PROGMEM = {
...
{
	value = map(value, 544, 2400, MIN_PULSE_WIDTH_PCA, MAX_PULSE_WIDTH_PCA);
	return value;
...
//------------------------------------------------------------------------------------------
//[BeginServoUpdate] Does whatever preperation that is needed to start a move of our servos
//------------------------------------------------------------------------------------------
...
//##################################UpdatedPCA##########################################
  ...
  pwm.setPWM(bServoIndex, 0, writeMicro2AdaPWM(wFemurSSCV + g_asLegOffsets[bServoIndex]));
  bServoIndex++;
...
void AllLegServos1500() 
{
    for (byte bServoIndex=0; bServoIndex < CNT_LEGS*NUMSERVOSPERLEG; bServoIndex++) {
		...
		pwm.setPWM(bServoIndex, 0, writeMicro2AdaPWM(1500 + g_asLegOffsets[bServoIndex]));
    }
}
